...
const LAST_RUN_FILE = path.join(__dirname, "lastRunPolizasSMG.json");
...
async function descargarYSubirPoliza({
  ...
  const url = "http://localhost:53692/api/exportar/poliza/impresion";
  const params = {
    usuario,
    password,
    productores,
...
async function procesarIntervalo(desde, hasta, acumuladas) {
  ...
  for (const tipo of tiposOp) {
    ...
    const response = await axios.get(
      "http://localhost:53692/api/exportar/libros",
      {
        params,
        responseType: "stream",
      }
    ...
    const fileNameCrudo = `polizas_crudo_${params.fec_desde}_${params.fec_hasta}_${tipo}.json`;
    fs.writeFileSync(fileNameCrudo, JSON.stringify(filas, null, 2));
    ...
    fs.writeFileSync(
      "polizas_totales.json",
      JSON.stringify(acumuladas, null, 2)
    );
...
async function obtenerTodasLasPolizasHistoricas() {
  ...
  const defaultStart = new Date("2025-06-12");
...
function guardarErroresEnJson(errores) {
  const filePath = path.join(__dirname, "errores-intervalos.json");
  try {
...
async function reprocesarErrores() {
  const erroresPath = path.join(__dirname, "errores_intervalos_smg.json");
